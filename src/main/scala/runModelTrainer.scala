...
object runModelTrainer {
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("runModelTrainer")
  ...
  def w2v(trainPath: String, executors: Int, spark: SparkSession): Unit ={
    ...
    new feature.Word2Vec().setInputCol("token").setOutputCol("features")
      .setVectorSize(200)
      .setWindowSize(10)
  ...
  def SkipHs(trainPath: String, executors: Int, spark: SparkSession): Unit = {
    ...
    val start = System.currentTimeMillis()
    val tokenizer = new Tokenizer().setInputCol("doc").setOutputCol("token")
    new Word2Vec().setInputCol("token").setOutputCol("features")
      .setCBOW(0)
      .setHs(true)
      .setNegative(0)
      .setSample(0.0001)
